...
class EntryCountCalculator
{
private:
	unsigned long long _count;

public:
	EntryCountCalculator() : _count(0)
	{
 ...
 *
 */
template<typename Key, typename Data, typename SizeCalculator = EntryCountCalculator> 
class LRUCache {
	public:
	...
	public:
		 ...
		 */
		LRUCache( const unsigned long long Size ) :
				_max_size( Size )
				{}
		 ...
		 */
		inline const typename List::size_type size( void ) {
			SCOPED_MUTEX;
			return _list.size(); 
		}
		 ...
		 *  @return size in elements
		 */
		inline const unsigned long long max_size( void ) const { return _max_size; }
		...
		inline void insert( const Key &key, const Data &data ) {
			...
			//tell the calculator
			_calculator.on_add(data);

			// Check to see if we need to remove an element due to exceeding max_size
			while ( _calculator.get_size() > _max_size ) {
				// Remove elements until the size is under the max size
				liter = _list.end();
				--liter;
	...
	private:
		...
		inline void _remove( const Map_Iter &miter ) {
			_calculator.on_remove(miter->second->second);
